...
import com.zafar.umair.rentout.model.ChatChannel
...
//            }
    ...
    override suspend fun updateRequestStatus(newstatus: String, reqId: String) {
        firestore.collection(RENTAL_COLLECTION).document(reqId).update("requestStatus", newstatus)
    }

    override suspend fun sendMessage(message: TextMessage, channelId: String) {
        firestore.collection(CHAT_CHANNELS).document(channelId)
            .collection(MESSAGES)
            .add(message)
    }

    override suspend fun getOrCreateChatChannel(
        otherUserId: String,
        ...
        firestore.collection(USER_COLLECTION).document(auth.currentUserId)
            ...
            .document(otherUserId).get().addOnSuccessListener {
                ...
                val newChannel = firestore.collection(CHAT_CHANNELS).document()
                newChannel.set(ChatChannel(mutableListOf(auth.currentUserId, otherUserId)))
                ...
                firestore.collection(USER_COLLECTION).document(otherUserId)
                    .collection(ENGAGED_CHAT_CHANNELS)
...
//                .orderBy("time").dataObjects()
    ...
    override suspend fun getChatMessages(
        ...
        return firestore.collection(CHAT_CHANNELS).document(channelId)
            .collection("messages")
            .orderBy("time")
            .dataObjects()
    }
    ...
    @OptIn(ExperimentalCoroutinesApi::class)
    override val favoriteProducts: Flow<List<FavoriteDocument>>
        get() =
            auth.currentUser.flatMapLatest { user ->
                firestore.collection(FAVORITE_COLLECTION).whereEqualTo(USER_ID_FIELD, user.id)
                    .dataObjects()
            }

    @OptIn(ExperimentalCoroutinesApi::class)
    override val othersRentalRequests: Flow<List<RentalRequest>>
        get() =
            auth.currentUser.flatMapLatest { user ->
                firestore.collection(RENTAL_COLLECTION).whereEqualTo(OWNER_ID_FIELD, user.id)
                    .dataObjects()
            }
    ...
    override val myRentalRequests: Flow<List<RentalRequest>>
        get() =
            auth.currentUser.flatMapLatest { user ->
                firestore.collection(RENTAL_COLLECTION).whereEqualTo(USER_ID_FIELD, user.id)
                    .dataObjects()
            }
...
//                    .whereNotEqualTo(RATED_USER_ID_FIELD, user.id)
    ...
    override suspend fun addProductToFavorites(productId: String) {
        trace(SAVE_FAVORITE_TRACE) {
            firestore.collection(FAVORITE_COLLECTION)
                .document(auth.currentUserId).get().addOnSuccessListener {
                    if (it.exists()) {
                        firestore.collection(FAVORITE_COLLECTION)
                            .document(auth.currentUserId)
...
//                        return@addOnSuccessListener
                        ...
                        firestore.collection(FAVORITE_COLLECTION)
                            .document(auth.currentUserId)
                            .set(FavoriteDocument(auth.currentUserId, mutableListOf(productId)))
                    }
                }
//                .update("productIds", FieldValue.arrayUnion(productId))
//                .update(FavoriteDocument(mutableListOf(productId)))
//                .set(FavoriteDocument(mutableListOf(productId)))
        }
    }
    ...
    override suspend fun saveReviewRating(ratingReview: RatingReview): String =
        trace(SAVE_RATING_REVIEW_TRACE) {
//            val productWithUserId = ratingReview.copy(userId = auth.currentUserId)
            firestore.collection(REVIEW_RATING_COLLECTION).add(ratingReview).await().id
        }

    override suspend fun saveRatingId(ratingId: String, rentalRequest: RentalRequest) {
        firestore.collection(RENTAL_COLLECTION).document(rentalRequest.id)
...
//      firestore.collection(USER_COLLECTION).add(userWithId).await().id
    ...
    override suspend fun updateUserFCM(token: String, userId: String) {
            firestore.collection(USER_COLLECTION).document(userId).update("fcmToken", token)
    }
    ...
    companion object {
        ...
        private const val SAVE_RATING_REVIEW_TRACE = "saveRatingReview"
